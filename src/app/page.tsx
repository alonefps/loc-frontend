...
const MapboxMap = dynamic(
  () => import('@/components/map/MapboxMap').then(mod => ({ default: mod.MapboxMap })),
  { ssr: false, loading: () => <LoadingSpinner size="lg" /> }
);

const RouteView = dynamic(
  () => import('@/components/route/RouteView').then(mod => ({ default: mod.RouteView })),
  { ssr: false, loading: () => <LoadingSpinner size="lg" /> }
);
  ...
  useEffect(() => {
    ...
    const fetchLocations = async () => {
      try {
        ...
        setError('Erro ao carregar locais. Verifique se o backend está rodando.');
      } finally {
        setIsLoading(false);
      }
    };
    ...
    <div className="min-h-screen bg-gradient-to-br from-neutral-950 to-neutral-900">
        ...
        <div className="px-3 sm:px-6 lg:px-8">
          <div className="flex h-14 sm:h-16 items-center justify-between gap-2">
            ...
            >
              ...
              <h1 className="text-base sm:text-xl font-bold gradient-text hidden xs:block">Locations</h1>
            </motion.div>
            ...
            >
              ...
              >
                ...
                <div className="flex flex-col items-start">
                  <span className="text-[9px] sm:text-xs text-neutral-400 leading-none">Nv.{userStats.level}</span>
                  <span className="text-[10px] sm:text-xs font-bold text-blue-400 leading-none">{userStats.points}</span>
                </div>
              </button>
      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        <div className="grid lg:grid-cols-12 gap-8">
          {/* Main content */}
          <div className={`${showProfile ? 'lg:col-span-9' : 'lg:col-span-12'}`}>
            {!showMap ? (
              <>
                <motion.section
                ...
                >
                  <div className="space-y-6">
                    ...
                    <p className="text-lg sm:text-xl text-neutral-400 max-w-xl">
                      Explore, avalie e compartilhe locais. Ganhe pontos e desbloqueie recompensas! 🎉
                    </p>
                ...
                <section id="locations" className="space-y-6 sm:space-y-8">
                  ...
                  >
                    ...
                    <p className="text-neutral-400">
                      {locations.length} {locations.length === 1 ? 'local disponível' : 'locais disponíveis'}
                    </p>
                  </motion.div>
